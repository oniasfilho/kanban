...
function TaskViewModal() {
  ...
  const handleSubtaskChange = (subtask) => {
    const updatedTask = {
      ...currentTask,
      subtasks: currentTask.subtasks.map((prevSubtask) => (prevSubtask.subTaskId === subtask.subTaskId
        ? { ...prevSubtask, isCompleted: !prevSubtask.isCompleted }
        : prevSubtask)),
    };
    dispatch(setCurrentTask(updatedTask));
    const updatedCurrentBoard = JSON.parse(JSON.stringify(currentBoard));
    ...
    if (differentColumns) {
      ...
      const updatedColumns = currentBoard.columns.map((column) => {
        if (column.columnId === departingId) {
          const updatedColumn = structuredClone(column);
