...
import { select, setGenericModal, setModalType, setCurrentTask, setLateralMenu } from '../features/content/contentSlice';
...
const MenuOptions = ({ className }) => {
	...
	const handleCreateNewBoardClick = () => {
		console.log("chegou em handleCreateNewBoardClick")
		...
		<div className="inner-lateral-menu-body">
			<div className={`navbar-modal ${className || ""}`}>
				...
				<div className="navbar-modal-content">
					{boards?.map(board => (
						<div key={board.boardId} className="navbar-modal-board-option" onClick={() => dispatch(select(board))}>
					...
					<div className="navbar-modal-board-option create-new-board" onClick={handleCreateNewBoardClick}>
